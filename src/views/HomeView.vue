...
<template>
  <section class="w-full flex flex-col p-2 gap-6 items-center" v-loading="loading" element-loading-text="服務讀取中，請稍候。">
    ...
    <article class="w-full max-w-[1280px] grid grid-cols-3 gap-2">
      <div v-for="item in projectList" class="card  flex flex-col border rounded cusuor-poniter cursor-pointer hover:shadow-lg" :key="'prj_'+item.id" @click="changePage(item.id)">
